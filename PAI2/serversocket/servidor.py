...
import logging
import os
from config import DB_PATH, MAX_INTENTOS, BLOQUEO_TIEMPO, LOG_DIR
    ...
    try:
        while True:
            ...
            if command == "REGISTER":
                ...
                logger.info(f"Mensaje recibido - Usuario: {username}")
                database.save_message(conn, username, message)
                connstream.sendall(b"MESSAGE_SENT")
                logger.debug(f"Mensaje enviado con éxito por: {username}")
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
    ...
    server_socket.listen(350) # Cola de escucha aumentada a 300
